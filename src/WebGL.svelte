<script lang="ts">
    ...
    onMount(() => {
        ...
        const geometry = createRoundedBoxGeometry(width * SCALE, height * SCALE, thickness * SCALE, roundness * SCALE);
    ...
    $: if (mesh) {
        if (
            ...
            mesh.geometry = createRoundedBoxGeometry(width * SCALE, height * SCALE, thickness * SCALE, roundness * SCALE);

            (mesh.material as THREE.MeshPhongMaterial[]).forEach((material: THREE.MeshPhongMaterial, i: number) => {
                material.color.set(colors[i]);
            });
        }
    }

    function createRoundedBoxGeometry(w: number, h: number, d: number, r: number) {
        ...
        geometry.addGroup(fabricSideIdxEnd, gp2.count - fabricSideCount - fabricSideOffsetCount, 1);
